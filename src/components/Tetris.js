...
const Tetris = () => {
    ...
    return (
        <StyledTetrisWrapper role="button" tabIndex={0} onKeyDown={move} onKeyUp={keyUp}>
            {pauseGame && <InfoOverlay text="Game Paused" onClick={pauseGameHandler} />}
            <StyledTetris>
                <Stage stage={stage} />
                <aside>
                    {gameOver ? (
                        <div>
                            <InfoOverlay text="Game Over!" onClick={startGame} />
                    ...
                    <Button callBack={pauseGameHandler} caption="Pause Game" disabled={!dropTime}/>
                </aside>
            </StyledTetris>
